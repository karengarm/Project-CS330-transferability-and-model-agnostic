...
def prepare_data_for_training_QA(train_dataset):
    ...
    for p in range(n_paragraph):
        if 'context' in train_dataset[p]:
            context = train_dataset[p]['context']
        if 'passage' in train_dataset[p]:
            context = train_dataset[p]['passage']
        if 'plot' in train_dataset[p]:
            context = train_dataset[p]['plot']
        ...
        if 'answers' in train_dataset[p]:
            answer = train_dataset[p]['answers']
            if 'text' not in answer:
                answer = {'text': answer}
                if len(answer['text']) > 0:
                    answer['answer_start'] = [context.find(answer['text'][0])]
        if 'answers_spans' in train_dataset[p]:
            answer = train_dataset[p]['answers_spans']
            answer['text'] = answer['spans']
            answer['answer_start'] = [context.find(answer['spans'][0])]
        if len(answer['text']) > 0:
